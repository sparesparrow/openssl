...
class OpenSSLToolsConan(ConanFile):
    ...
    def configure(self):
        ...
        if self.options.vcpkg_integration:
            ...
            if vcpkg_root:
                ...
                # Note: Cannot modify options in configure(), will check in build()
            else:
    ...
    def build(self):
        ...
        # Check if vcpkg should be preferred
        prefer_vcpkg = False
        if self.options.vcpkg_integration:
            vcpkg_root = self._detect_vcpkg_root()
            if vcpkg_root:
        ...
        if self.options.vcpkg_integration and prefer_vcpkg:
            # Try vcpkg first
...
"""
    ...
    def package(self):
        ...
        config = {
            ...
            "vcpkg_root": self._detect_vcpkg_root(),
            "vcpkg_triplet": str(self._vcpkg_triplet) if hasattr(self, '_vcpkg_triplet') and self._vcpkg_triplet else None
        }
